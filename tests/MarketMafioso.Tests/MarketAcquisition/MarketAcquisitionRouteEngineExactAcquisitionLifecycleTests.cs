...
public sealed class MarketAcquisitionRouteEngineExactAcquisitionLifecycleTests
{
    [Fact]
    public void InactiveRoute_ClearsPausedStateWithoutFinalizedContract()
    {
        ...
        Assert.True(ExactAcquisitionRouteRecoveryLifecycle.ClearOrphanedState(
            isRouteActive: false,
            store.Restore(),
    ...
    [Theory]
    [InlineData(ExactAcquisitionRouteAuthorityPhase.Paused)]
    [InlineData(ExactAcquisitionRouteAuthorityPhase.RecoveryNeeded)]
    public void InactiveRoute_PreservesStateWithMatchingFinalizedContract(ExactAcquisitionRouteAuthorityPhase phase)
    {
        var fixture = Fixture(dryRunOnly: true, splitListings: true);
        var store = new MemoryStore();
        ...
        Assert.False(ExactAcquisitionRouteRecoveryLifecycle.ClearOrphanedState(
            isRouteActive: false,
            persisted,
            fixture.Contract,
            store));
    ...
    [Fact]
    public void OrdinaryRouteWithoutExactAcquisitionState_PerformsNoRepair()
    {
        var store = new MemoryStore();
    ...
    public void ChangedVisibleCoverage_StopsOriginalRunnerAndStartsRecoveredPlanWithoutGenericResume()
    {
        ...
        var rejection = harness.Engine.EnforceExactAcquisitionCandidateAuthority(
        ...
        Assert.Equal(ExactAcquisitionRouteAuthorityPhase.RecoveryNeeded, recovery.ExactAcquisitionExecution!.Phase);
        Assert.False(recovery.IsPaused);
        Assert.Equal("Stopped", harness.Runner.State);
        Assert.Equal(
            MarketAcquisitionGuidedRoutePrimaryAction.RetryExactAcquisitionRecovery,
            MarketAcquisitionGuidedRouteActionPresenter.Resolve(recovery));

        var remaining = harness.Engine.CreateExactAcquisitionRecoveryClaim(fixture.Claim);
        var recoveredPlan = WithListingId(fixture.Plan, "recovered-2");
        var result = harness.Engine.StartExactAcquisitionRecovery(recoveredPlan, remaining, fixture.Document);

        Assert.True(result.Success);
        var active = harness.Engine.CreateSnapshot();
        Assert.Equal(ExactAcquisitionRouteAuthorityPhase.Active, active.ExactAcquisitionExecution!.Phase);
    ...
    public void ManualPause_OffersExplicitResumeAndDoesNotEnterRecovery()
    {
        ...
        Assert.Equal(ExactAcquisitionRouteAuthorityPhase.Paused, paused.ExactAcquisitionExecution!.Phase);
        ...
        Assert.Equal(ExactAcquisitionRouteAuthorityPhase.Active, resumed.ExactAcquisitionExecution!.Phase);
    ...
    public void NoPlanAutoResume_PausesOnceAndIsNotEligibleForAnotherAutomaticAttempt()
    {
        var fixture = Fixture();
        var store = new MemoryStore();
        var session = ExactAcquisitionRouteAuthoritySession.Consume(
            fixture.Contract,
            fixture.Document,
            fixture.Plan,
            fixture.Claim,
        ...
        var paused = ExactAcquisitionRouteRecoveryLifecycle.PauseUnavailable(
            session.State,
            "No viable exact-quality route remains.",
            DateTimeOffset.UnixEpoch);
        store.Save(paused);

        Assert.Equal(ExactAcquisitionRouteAuthorityPhase.Paused, store.Restore()!.Phase);
        Assert.False(ExactAcquisitionRouteRecoveryLifecycle.CanAutoResume(store.Restore()));
    }
    ...
    public void DiagnosticChangedRowScenario_EntersRecoveryOnceAndCanResumeRecoveredPlan()
    {
        var fixture = Fixture(dryRunOnly: true);
        using var harness = MarketAcquisitionRouteEngineHarness.Create(new MemoryStore());
        Assert.True(harness.Engine.ArmExactAcquisitionDryRunScenario(ExactAcquisitionDryRunScenario.ChangedListingRecovery));
        Assert.True(harness.Engine.Start(
            fixture.Plan,
            fixture.Claim,
            false,
        ...
        var injected = harness.Engine.EnforceExactAcquisitionCandidateAuthority(
            fixture.Plan.WorldBatches[0].ItemSubtasks[0],
            CandidatePlan());

        Assert.NotNull(injected);
        Assert.Equal(ExactAcquisitionRouteAuthorityPhase.RecoveryNeeded, harness.Engine.CreateSnapshot().ExactAcquisitionExecution!.Phase);
        var remaining = harness.Engine.CreateExactAcquisitionRecoveryClaim(fixture.Claim);
        Assert.True(harness.Engine.StartExactAcquisitionRecovery(fixture.Plan, remaining, fixture.Document).Success);
        Assert.Null(harness.Engine.EnforceExactAcquisitionCandidateAuthority(
            fixture.Plan.WorldBatches[0].ItemSubtasks[0],
            CandidatePlan()));
    }
    ...
    public void DiagnosticNoViableScenario_IsConsumedOnceOnlyAfterRecoveryIsNeeded()
    {
        var fixture = Fixture(dryRunOnly: true);
        using var harness = MarketAcquisitionRouteEngineHarness.Create(new MemoryStore());
        Assert.True(harness.Engine.ArmExactAcquisitionDryRunScenario(ExactAcquisitionDryRunScenario.NoViableRecovery));
        Assert.True(harness.Engine.Start(
            fixture.Plan,
            fixture.Claim,
            false,
            ...
            fixture.Document,
            MarketAcquisitionExecutionMode.DryRun).Success);
        Assert.False(harness.Engine.ConsumeNoViableExactAcquisitionDryRunScenario());

        Assert.NotNull(harness.Engine.EnforceExactAcquisitionCandidateAuthority(
            fixture.Plan.WorldBatches[0].ItemSubtasks[0],
            CandidatePlan()));

        Assert.True(harness.Engine.ConsumeNoViableExactAcquisitionDryRunScenario());
        Assert.False(harness.Engine.ConsumeNoViableExactAcquisitionDryRunScenario());
    }
    ...
    public void FailedRecoveryPreflight_PausesInsteadOfRemainingRecoveryEligible()
    {
        ...
        var result = harness.Engine.StartExactAcquisitionRecovery(
    ...
    public void RestartDryRun_RestoresSunkReceiptOnceWithoutMutatingStoreOrAttemptingPurchase()
    {
        ...
        var saveCount = 0;
        var store = new ConfigurationExactAcquisitionRouteExecutionStateStore(config, () => saveCount++);
        var seeded = SeededState(fixture);
        store.Save(seeded);
        var persistedJson = config.ExactAcquisitionRouteExecutionStateJson;
        ...
        using (var first = MarketAcquisitionRouteEngineHarness.Create(store))
        {
            ...
            Assert.Equal(1u, snapshot.ExactAcquisitionExecution!.Lines[0].PurchasedQuantity);
            Assert.Equal(100ul, snapshot.ExactAcquisitionExecution.TotalSpentGil);
            Assert.Same(snapshot.ActivePlan, ExactAcquisitionDryRunExecutionStateRestorer.RestoreRemainingPlan(
                fixture.Contract,
                fixture.Document,
                fixture.Claim,
                snapshot.ActivePlan,
            ...
            Assert.Equal(persistedJson, config.ExactAcquisitionRouteExecutionStateJson);
    ...
    public void RestartDryRun_AuthorityRejectsSunkCandidatePassedAfterPlanning()
    {
        ...
        Assert.Equal(
            ...
            harness.Engine.CreateSnapshot().ExactAcquisitionExecution!.Phase);
    }
    ...
    private static FixtureData Fixture(bool dryRunOnly = false, bool splitListings = false)
    {
        ...
        var sourceTransfer = ExactAcquisitionWorkbenchAuthorityTests.Transfer();
        ...
        return new(document, document.ExactAcquisitionAuthority!.FinalizedContract!, claim, plan);
    }
    ...
    private static ExactAcquisitionRouteExecutionState SeededState(
        FixtureData fixture,
        Func<ExactAcquisitionRouteSunkPurchase, ExactAcquisitionRouteSunkPurchase>? mutateReceipt = null)
    {
        ...
        receipt = receipt with { ReceiptId = ExactAcquisitionDryRunExecutionStateRestorer.ComputeReceiptId(receipt) };
        return new ExactAcquisitionRouteExecutionState(
            ExactAcquisitionRouteExecutionState.CurrentSchemaVersion,
            fixture.Contract.ContractId,
            fixture.Contract.CanonicalIntentHash,
            ExactAcquisitionRouteAuthorityPhase.Paused,
            [new ExactAcquisitionRouteLineProgress("line-1", line.ItemId, line.ItemName, line.Quality, 2, 1, 100, 100, 200)],
            100,
            0,
            null,
            0,
    ...
    private sealed class MemoryStore : IExactAcquisitionRouteExecutionStateStore
    {
        private ExactAcquisitionRouteExecutionState? state;
        public ExactAcquisitionRouteExecutionState? State => state;
        public int SaveCount { get; private set; }
        public int ClearCount { get; private set; }
        public ExactAcquisitionRouteExecutionState? Restore() => state;
        public void Save(ExactAcquisitionRouteExecutionState value)
        {
            SaveCount++;
            state = value;
        }
