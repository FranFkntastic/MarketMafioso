...
public sealed class RenderedCharacterStatsParserTests
{
    ...
    [Fact]
    public void ParseCrafting_PairsVisibleCraftingLabelsWithValuesInTheirRenderedComponentRows()
    {
        var capturedAt = DateTimeOffset.Parse("2026-07-19T11:30:00Z");
        var snapshot = new AgentBridgeRenderedUiSnapshot(capturedAt,
        [
            Addon("Character",
                Node("Character/65", "Blacksmith", 0, 0),
                Node("Character/64", "Level 100", 0, 20)),
            Addon("CharacterStatus",
                Node("CharacterStatus/78/2", "Craftsmanship", 0, 0),
                Node("CharacterStatus/78/3", "5831", 118, 0),
                Node("CharacterStatus/78/2", "excl. Consumables", 0, 20),
                Node("CharacterStatus/78/3", "5831", 118, 20),
                Node("CharacterStatus/79/2", "Control", 0, 0),
                Node("CharacterStatus/79/3", "5,481", 118, 0),
                Node("CharacterStatus/79/2", "CP", 0, 40),
                Node("CharacterStatus/79/3", "664", 118, 40)),
        ]);

        var result = RenderedCharacterStatsParser.ParseCrafting(snapshot);

        Assert.Equal(RenderedCharacterObservationStatus.Complete, result.Status);
        Assert.Equal("Blacksmith", result.JobName);
        Assert.Equal(100, result.Level);
        Assert.Equal(5831, result.Craftsmanship);
        Assert.Equal(5481, result.Control);
        Assert.Equal(664, result.CraftingPoints);
        Assert.Equal(3, result.Evidence.Count);
    }

    [Fact]
    public void ParseCrafting_AbstainsWhenActiveJobIsNotACrafter()
    {
        var snapshot = new AgentBridgeRenderedUiSnapshot(DateTimeOffset.UtcNow,
        [
            Addon("Character", Node("Character/65", "Miner", 0, 0)),
        ]);

        var result = RenderedCharacterStatsParser.ParseCrafting(snapshot);

        Assert.Equal(RenderedCharacterObservationStatus.Unavailable, result.Status);
        Assert.Contains("not a crafting job", result.Diagnostic);
    }

    [Fact]
    public void CraftingStabilizer_RequiresACompleteRenderedTupleToRemainUnchanged()
    {
        var stabilizer = new RenderedCraftingStatsStabilizer(TimeSpan.FromSeconds(3));
        var first = new RenderedCraftingStatsObservation(
            Guid.NewGuid(),
        ...
        Assert.Equal(RenderedCharacterObservationStatus.Partial, stabilizer.Observe(first).Status);
        Assert.Equal(RenderedCharacterObservationStatus.Partial, stabilizer.Observe(first with { CapturedAtUtc = first.CapturedAtUtc.AddSeconds(2.9) }).Status);
        Assert.Equal(RenderedCharacterObservationStatus.Complete, stabilizer.Observe(first with { CapturedAtUtc = first.CapturedAtUtc.AddSeconds(3.1) }).Status);
    }
