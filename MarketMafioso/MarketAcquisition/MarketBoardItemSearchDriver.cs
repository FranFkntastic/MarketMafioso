...
public sealed class MarketBoardItemSearchDriver
{
    ...
    public unsafe MarketBoardItemSearchResult CaptureInputSnapshot(uint itemId, string? itemName)
    {
        var details = new Dictionary<string, string?>
        {
            ["expectedItemId"] = itemId == 0 ? null : itemId.ToString(CultureInfo.InvariantCulture),
        ...
        var addon = gameGui.GetAddonByName<AddonItemSearch>(ItemSearchAddon, 1);
        details["itemSearchAddonAvailable"] = (addon != null).ToString();
        if (addon == null)
        {
            return new MarketBoardItemSearchResult
            {
                Status = "InputSnapshot",
                Message = "Captured market board input state; item search addon is not available.",
                Details = details,
            };
        }

        details["itemSearchAddonReady"] = addon->AtkUnitBase.IsReady.ToString();
        details["itemSearchAddonVisible"] = addon->AtkUnitBase.IsVisible.ToString();
        details["mode"] = FormatMode((uint)addon->Mode);
        details["modeRaw"] = ((uint)addon->Mode).ToString(CultureInfo.InvariantCulture);
        details["partialSearch"] = addon->PartialMatch.ToString();
        details["addonSearchText"] = addon->SearchText.ToString();
        details["addonSearchText2"] = addon->SearchText2.ToString();
        details["addonFocusNode"] = FormatNode(addon->AtkUnitBase.FocusNode);
        details["addonComponentFocusNode"] = FormatNode(addon->AtkUnitBase.ComponentFocusNode);

        var searchButton = addon->SearchButton;
        details["searchButtonAvailable"] = (searchButton != null).ToString();
        details["searchButtonEnabled"] = (searchButton != null && searchButton->IsEnabled).ToString();
        if (searchButton != null)
            details["searchButtonOwnerNode"] = FormatNode(searchButton->AtkComponentBase.OwnerNode == null ? null : &searchButton->AtkComponentBase.OwnerNode->AtkResNode);
        ...
        if (input != null)
        {
            ...
            details["textInputOwnerNode"] = FormatNode(ownerNode == null ? null : &ownerNode->AtkResNode);
            details["textInputFocusNode"] = FormatNode(inputBase->AtkComponentBase.GetFocusNode());
            details["textInputIsActive"] = inputBase->IsActive.ToString();
            details["textInputCursorPos"] = inputBase->CursorPos.ToString(CultureInfo.InvariantCulture);
            details["textInputSelectionStart"] = inputBase->SelectionStart.ToString(CultureInfo.InvariantCulture);
            details["textInputSelectionEnd"] = inputBase->SelectionEnd.ToString(CultureInfo.InvariantCulture);
            details["textInputCallbackAvailable"] = (inputBase->Callback != null).ToString();
            details["textInputCallbackEventKind"] = inputBase->CallbackEventKind.ToString(CultureInfo.InvariantCulture);
            details["textInputRawString"] = inputBase->RawString.ToString();
            details["textInputEvaluatedString"] = inputBase->EvaluatedString.ToString();
            details["textInputTextNodeText"] = inputBase->AtkTextNode == null
                ? null
                : inputBase->AtkTextNode->NodeText.ToString();
            details["textInputCollisionNode"] = FormatNode(inputBase->CollisionNode == null ? null : &inputBase->CollisionNode->AtkResNode);
        }
        ...
        if (stage != null)
        {
            ...
            {
                details["inputManagerFocusedNode"] = FormatNode(stage->AtkInputManager->FocusedNode);
                details["inputManagerTextInput"] = FormatPointer(stage->AtkInputManager->TextInput);
            }
        }
        ...
        AddAgentDetails(details, agent);
        if (itemId != 0)
            details["exactItemVisibleInAgent"] = AgentContainsItem(agent, itemId).ToString();
