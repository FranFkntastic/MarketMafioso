...
Any valid scoped machine key may read `/api/capabilities` so a narrow client can discover whether its feature is available. A `CraftQuoteApiKey` cannot read inventory reports.
