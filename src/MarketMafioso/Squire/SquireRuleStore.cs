...
public sealed class SquireRuleStore
{
    ...
    public SquireRuleStore(Configuration config, Action? save = null)
    {
...
{
    public static bool Migrate(Configuration config)
    {
        config.Squire ??= new SquireConfiguration();
        var changed = Migrate(config.Squire);
        if (config.SettingsSelectedPageId is "squire.exclusions" or "squire.duplicates")
        {
            config.SettingsSelectedPageId = "squire.rules";
            changed = true;
        }
        return changed;
    }
