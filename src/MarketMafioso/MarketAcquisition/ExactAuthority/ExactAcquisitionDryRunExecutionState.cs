...
public sealed record ExactAcquisitionRouteSunkPurchase
{
    public const string CurrentSchemaVersion = "marketmafioso-squire-exactAcquisition-sunk-purchase/v1";
...
public sealed class RestoreOnlyExactAcquisitionRouteExecutionStateStore : IExactAcquisitionRouteExecutionStateStore
{
    private readonly IExactAcquisitionRouteExecutionStateStore source;

    public RestoreOnlyExactAcquisitionRouteExecutionStateStore(IExactAcquisitionRouteExecutionStateStore source) =>
        this.source = source ?? throw new ArgumentNullException(nameof(source));

    public ExactAcquisitionRouteExecutionState? Restore() => source.Restore();
    public void Save(ExactAcquisitionRouteExecutionState state) { }
    public void Clear() { }
}

public static class ExactAcquisitionDryRunPreparedPlanRestorer
{
        ...
        ExactAcquisitionRouteExecutionState persisted)
    {
        ...
        if (preparedAt == default)
            throw new InvalidOperationException("Persisted External plan sunk receipts do not identify one prepared plan generation.");
        if (contract.Transfer.Evidence.PublishedAtUtc > preparedAt)
            throw new InvalidOperationException("Persisted External plan listing evidence was published after the identified plan generation.");

        var fullPlan = BuildPinnedPlan(contract, claim, preparedAt, persisted.SunkPurchases);
        return ExactAcquisitionDryRunExecutionStateRestorer.RestoreRemainingPlan(
            contract,
            document,
            claim,
            fullPlan,
    ...
    private static MarketAcquisitionPlan BuildPinnedPlan(
        ...
        MarketAcquisitionClaimView claim,
        DateTimeOffset preparedAt,
        IReadOnlyList<ExactAcquisitionRouteSunkPurchase>? legacyReceipts)
    {
        if (preparedAt == default || contract.Transfer.Evidence.PublishedAtUtc > preparedAt)
            throw new InvalidOperationException("Finalized External plan listing evidence does not precede one valid plan generation.");
        ...
        if (contractLines.Count != contract.Lines.Count || claimLines.Count != claim.Lines.Count ||
            ...
            throw new InvalidOperationException("The finalized External plan contract and accepted claim do not have one exact line mapping.");
        ...
        var resolved = contract.Transfer.MarketLots.Select(lot =>
        {
            if (lot.OfferKey.SourceKind != EquipmentAcquisitionSourceKind.MarketBoard ||
                ...
                throw new InvalidOperationException("Persisted External plan listing authority is incomplete or outside its finalized envelope.");
            ...
            if (string.IsNullOrWhiteSpace(retainerId))
            {
                ...
                if (receiptRetainers.Length != 1)
                    throw new InvalidOperationException("Persisted External plan listing authority lacks an exact retainer identity; prepare a new dry-run plan.");
        ...
        if (resolved.Select(LotIdentity).Distinct(StringComparer.Ordinal).Count() != resolved.Length)
            throw new InvalidOperationException("Persisted External plan listing authority contains duplicate listing identities.");
        foreach (var pair in contractLines)
        {
            ...
            if (quantity != pair.Value.RequiredQuantity || gil > pair.Value.MaxTotalGil)
                throw new InvalidOperationException($"Persisted External plan listing authority does not exactly fulfill {pair.Value.ItemName} within its gil cap.");
        }
        var observedTotal = resolved.Aggregate(0ul, (sum, value) => checked(sum + value.Lot.ObservedTotalPriceGil));
        if (observedTotal != contract.Transfer.ObservedMarketTotalGil || observedTotal > contract.PlanCapGil)
            throw new InvalidOperationException("Persisted External plan listing totals disagree with the finalized transfer or exceed its plan cap.");
        ...
        if (!plannedIdentities.SequenceEqual(expectedIdentities, StringComparer.Ordinal))
            throw new InvalidOperationException("Reconstructed external exact-acquisition plan does not preserve every exact finalized listing row.");
        return fullPlan;
    }
        ...
        ExactAcquisitionRouteExecutionState persisted)
    {
        if (persisted.SunkPurchases is not { Count: > 0 })
            throw new InvalidOperationException("Persisted External plan sunk receipts are unavailable.");
        ValidateContractIdentity(contract, document, claim);
        ...
        if (persisted.Phase != ExactAcquisitionRouteAuthorityPhase.Paused ||
            ...
            !ExactAcquisitionRouteAuthoritySession.IsRestoredStateSane(persisted, contract, claim))
            throw new InvalidOperationException("Persisted External plan sunk state or listing evidence is unavailable, unbounded, active, or mismatched.");
    }
    ...
    private static void ValidateContractIdentity(
        ...
        MarketAcquisitionClaimView claim)
    {
        var finalized = document.ExactAcquisitionAuthority?.FinalizedContract;
        if (!contract.Transfer.DryRunOnly ||
            contract.SchemaVersion != ExactAcquisitionExecutionContract.CurrentSchemaVersion ||
            contract.Transfer.SchemaVersion != ExactAcquisitionWorkbenchTransfer.CurrentSchemaVersion ||
            contract.Transfer.Origin != ExactAcquisitionWorkbenchTransfer.ExternalPlanningOrigin ||
            ...
            ExactAcquisitionWorkbenchAuthorityService.ComputeCanonicalIntentHash(document) != contract.CanonicalIntentHash ||
    ...
    private static EquipmentQuality QualityFromPolicy(string policy) =>
        MarketAcquisitionPolicy.NormalizeHqPolicy(policy) switch
        {
            ...
            _ => throw new InvalidOperationException("External plan startup restoration requires exact NQ or HQ claim authority."),
        };
    ...
    private sealed record ResolvedLot(
        string LineId,
        ExactAcquisitionWorkbenchMarketLot Lot,
        string RetainerName,
        string RetainerId);
}

public static class ExactAcquisitionDryRunExecutionStateRestorer
{
        ...
        ExactAcquisitionRouteExecutionState? persisted)
    {
        ...
        if (persisted is null)
            return plan;
        if (!ExactAcquisitionRouteAuthoritySession.IsRestoredStateSane(persisted, contract, claim))
            throw new InvalidOperationException("Persisted External plan sunk state does not exactly match the finalized contract and line envelopes.");
        if (persisted.SunkPurchases is null)
            throw new InvalidOperationException("Persisted External plan sunk receipt collection is unavailable.");
        ...
        if (purchasedQuantity == 0 && persisted.TotalSpentGil == 0)
        {
            if (persisted.SunkPurchases.Count != 0)
                throw new InvalidOperationException("Persisted External plan sunk receipts exist without matching quantity and gil totals.");
            return plan;
        }
        if (persisted.SunkPurchases.Count == 0)
            throw new InvalidOperationException("Persisted External plan sunk totals have no world/listing receipts and cannot authorize dry-run restoration.");
            ...
            var subtasks = batch.ItemSubtasks.Select(subtask =>
            {
                var listings = subtask.Listings.Select(listing =>
                {
                    ...
                    if (!matched.Add(key) || deduction.Quantity > listing.Quantity ||
                        ...
                        throw new InvalidOperationException("Persisted External plan sunk receipt exceeds or ambiguously matches its exact planned listing.");
        ...
        if (matched.Count != deductions.Count)
            throw new InvalidOperationException("Persisted External plan sunk receipt does not identify one exact listing in the prepared route.");
    ...
    public static string ComputeReceiptId(ExactAcquisitionRouteSunkPurchase receipt)
    {
    ...
    internal static void ValidateReceipt(
        ...
        MarketAcquisitionClaimView claim,
        MarketAcquisitionPlan plan,
        ExactAcquisitionRouteSunkPurchase receipt)
    {
        ValidateReceiptIdentity(contract, document, claim, receipt);
        if (plan.RequestId != claim.Id || receipt.PlanRequestId != plan.RequestId || receipt.PlanPreparedAtUtc != plan.PreparedAtUtc)
            throw new InvalidOperationException("Persisted External plan sunk receipt is stale or bound to different contract, Workbench, or plan evidence.");
        ...
        if (contractLine is null || planLine is null || listing is null ||
            ...
            receipt.TotalGil > contract.PlanCapGil)
            throw new InvalidOperationException("Persisted External plan sunk receipt does not match one exact line, listing, quantity, quality, and gil envelope.");
    }
    ...
    private static void ValidateReceiptIdentity(
        ...
        ExactAcquisitionRouteSunkPurchase receipt)
    {
        if (receipt.SchemaVersion != ExactAcquisitionRouteSunkPurchase.CurrentSchemaVersion ||
            ...
            ExactAcquisitionWorkbenchAuthorityService.ComputeCanonicalIntentHash(document) != contract.CanonicalIntentHash)
            throw new InvalidOperationException("Persisted External plan sunk receipt is stale or bound to different contract or Workbench evidence.");
        if (!contract.AuthorizedWorlds.Contains(receipt.WorldName, StringComparer.OrdinalIgnoreCase))
            throw new InvalidOperationException("Persisted External plan sunk receipt names a world outside the finalized contract.");
        ...
        if (claimLine is null || contractLine is null || claimLine.ItemId != receipt.ItemId ||
            ...
            receipt.TotalGil > contractLine.MaxTotalGil || receipt.TotalGil > contract.PlanCapGil)
            throw new InvalidOperationException("Persisted External plan sunk receipt does not match one exact contract line, quantity, quality, and gil envelope.");
    }

    private static bool IsAlreadyRemainingPlan(
        MarketAcquisitionPlan plan,
        ExactAcquisitionRouteExecutionState state) => state.Lines.All(line =>
    {
        ...
        MarketAcquisitionPlan plan)
    {
        ...
        foreach (var line in state.Lines)
        {
            ...
            if (plannedLine is null || plannedLine.ItemId != line.ItemId ||
                ...
                throw new InvalidOperationException($"Restored External plan route exceeds the exact remaining quantity or gil envelope for {line.ItemName}.");
            if (listings.Any(listing =>
                    ...
                    listing.TotalGil != checked(listing.Quantity * listing.UnitPrice)))
                throw new InvalidOperationException($"Restored External plan route changed the exact listing, quality, quantity, or gil identity for {line.ItemName}.");
            plannedExternalGil = checked(plannedExternalGil + plannedGil);
        }
        if (plannedExternalGil > contract.PlanCapGil - state.TotalSpentGil)
            throw new InvalidOperationException("Restored External plan route exceeds the exact remaining finalized plan envelope.");
    }

    private static void ValidateReceiptTotals(ExactAcquisitionRouteExecutionState state)
    {
        if (state.SunkPurchases.Select(receipt => receipt.ReceiptId).Distinct(StringComparer.Ordinal).Count() != state.SunkPurchases.Count)
            throw new InvalidOperationException("Persisted External plan sunk receipts contain duplicate evidence identities.");
        foreach (var line in state.Lines)
        {
            ...
            if (receipts.Aggregate(0ul, (sum, receipt) => checked(sum + receipt.Quantity)) != line.PurchasedQuantity ||
                ...
                throw new InvalidOperationException("Persisted External plan sunk receipts disagree with their line quantity or gil totals.");
        }
        if (state.SunkPurchases.Aggregate(0ul, (sum, receipt) => checked(sum + receipt.TotalGil)) != state.TotalSpentGil)
            throw new InvalidOperationException("Persisted External plan sunk receipts disagree with the route gil total.");
    }

    private static string ReceiptKey(ExactAcquisitionRouteSunkPurchase receipt) =>
