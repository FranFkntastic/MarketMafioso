...
public enum ExactAcquisitionRouteAuthorityPhase
{
    Preparing,
    Active,
    RecoveryNeeded,
...
public sealed record ExactAcquisitionRouteLineProgress(
    string LineId,
    uint ItemId,
    string ItemName,
...
public sealed record ExactAcquisitionRouteExecutionState(
    string SchemaVersion,
    string ContractId,
    string CanonicalIntentHash,
    ExactAcquisitionRouteAuthorityPhase Phase,
    IReadOnlyList<ExactAcquisitionRouteLineProgress> Lines,
    ulong TotalSpentGil,
    int RecoveryAttempt,
...
{
    public const string CurrentSchemaVersion = "marketmafioso-squire-exactAcquisition-route-state/v1";
    public bool NeedsRecovery => Phase == ExactAcquisitionRouteAuthorityPhase.RecoveryNeeded;
    public IReadOnlyList<ExactAcquisitionRouteSunkPurchase> SunkPurchases { get; init; } = [];
}

public interface IExactAcquisitionRouteExecutionStateStore
{
    ExactAcquisitionRouteExecutionState? Restore();
    void Save(ExactAcquisitionRouteExecutionState state);
    void Clear();
}

public sealed class ConfigurationExactAcquisitionRouteExecutionStateStore : IExactAcquisitionRouteExecutionStateStore
{
    private readonly Configuration config;
    private readonly Action save;

    public ConfigurationExactAcquisitionRouteExecutionStateStore(Configuration config, Action? save = null)
    {
        this.config = config ?? throw new ArgumentNullException(nameof(config));
        this.save = save ?? config.Save;
    }

    public ExactAcquisitionRouteExecutionState? Restore()
    {
        if (string.IsNullOrWhiteSpace(config.ExactAcquisitionRouteExecutionStateJson))
            return null;
        try
        {
            var state = JsonConvert.DeserializeObject<ExactAcquisitionRouteExecutionState>(config.ExactAcquisitionRouteExecutionStateJson);
            return state?.SchemaVersion == ExactAcquisitionRouteExecutionState.CurrentSchemaVersion ? state : null;
        }
        catch (JsonException)
        {
            return null;
    ...
    {
        config.ExactAcquisitionRouteExecutionStateJson = JsonConvert.SerializeObject(state, Formatting.None);
        save();
    }

    public void Clear()
    {
        config.ExactAcquisitionRouteExecutionStateJson = null;
        save();
    }
}

public sealed class ExactAcquisitionRouteAuthoritySession
{
    private readonly ExactAcquisitionExecutionContract contract;
    private readonly IExactAcquisitionRouteExecutionStateStore? store;

    private ExactAcquisitionRouteAuthoritySession(
    ...
    public ExactAcquisitionRouteExecutionState State { get; private set; }
    public ExactAcquisitionExecutionContract Contract => contract;

    public static ExactAcquisitionRouteAuthoritySession Consume(
        ExactAcquisitionExecutionContract contract,
        MarketAcquisitionRequestDocument document,
        MarketAcquisitionPlan plan,
        MarketAcquisitionClaimView claim,
        IExactAcquisitionRouteExecutionStateStore? store = null)
    {
        ...
        var state = restored is not null &&
                    ...
                    IsRestoredStateSane(restored, bindings, contract.PlanCapGil)
            ? restored with
            {
                Phase = ExactAcquisitionRouteAuthorityPhase.Preparing,
                Message = "Reconciled persisted purchases; preparing the remaining exact-quality route.",
                UpdatedAtUtc = DateTimeOffset.UtcNow,
            }
            : new ExactAcquisitionRouteExecutionState(
                ExactAcquisitionRouteExecutionState.CurrentSchemaVersion,
                contract.ContractId,
                contract.CanonicalIntentHash,
                ExactAcquisitionRouteAuthorityPhase.Preparing,
                bindings,
                0,
                0,
                null,
        ...
        catch (Exception exception)
        {
            store?.Save(state with
            {
                ...
                Message = $"External plan preflight stopped before travel or purchase: {exception.Message}",
                UpdatedAtUtc = DateTimeOffset.UtcNow,
            });
            throw;
        }
        var session = new ExactAcquisitionRouteAuthoritySession(contract, state, store);
        session.Persist();
        return session;
    }
    ...
    public void CompletePreflight(MarketAcquisitionPlan plan)
    {
        ...
        SetState(State with
        {
            Phase = ExactAcquisitionRouteAuthorityPhase.Active,
    ...
    public void ValidateCurrentDocument(MarketAcquisitionRequestDocument document)
    {
        ...
        if (document.LocalRequestId != contract.WorkbenchDocumentId ||
            ...
            throw new InvalidOperationException("Workbench changed after External plan confirmation; return through Advisor and finalize a new contract.");
    }
    ...
    public ExactAcquisitionWorkbenchAuthorityValidation AuthorizeCandidate(
        ...
        MarketAcquisitionLiveCandidatePlan candidatePlan)
    {
        ...
        if (State.Phase != ExactAcquisitionRouteAuthorityPhase.Active)
            return new(false, $"exact-acquisition authority is {State.Phase}; purchases are disabled.");
        ...
        if (MarketAcquisitionLiveCandidateStatuses.IsIncompleteListingCoverage(candidatePlan.Status))
            return new(false, "Visible listing coverage is incomplete; External plan paused instead of treating hidden rows as absent.");
        ...
        if (candidateGil > contract.PlanCapGil - State.TotalSpentGil)
            return new(false, "The visible listing would exceed the confirmed external exact-acquisition plan ceiling.");
        ...
        MarketAcquisitionPlan? plan = null)
    {
        ...
        ExactAcquisitionRouteSunkPurchase? receipt = null;
        if (plan is not null)
        {
            ...
            receipt = draft with { ReceiptId = ExactAcquisitionDryRunExecutionStateRestorer.ComputeReceiptId(draft) };
            ...
            if (existingReceipts.Any(value =>
                ...
                throw new InvalidOperationException("Confirmed listing already has a different persisted External plan receipt.");
        }
        if (candidate.ItemId != line.ItemId || candidate.IsHq != (line.Quality == EquipmentQuality.High) ||
            ...
            candidate.TotalGil > contract.PlanCapGil - State.TotalSpentGil)
        {
            throw new InvalidOperationException("Confirmed purchase is outside the finalized External plan execution contract.");
        }
    ...
    public void EvaluateRouteEnd(MarketAcquisitionPlan completedPlan)
    {
        ...
        if (State.Lines.All(line => line.PurchasedQuantity >= line.RequiredQuantity))
        {
            SetState(State with
            {
                ...
                Message = "The finalized exact-quality external exact-acquisition solution is fully acquired.",
                UpdatedAtUtc = DateTimeOffset.UtcNow,
            });
            return;
        }
        ...
        SetState(State with
        {
            Phase = repeatedWithoutProgress ? ExactAcquisitionRouteAuthorityPhase.Paused : ExactAcquisitionRouteAuthorityPhase.RecoveryNeeded,
    ...
    {
        Phase = ExactAcquisitionRouteAuthorityPhase.RecoveryNeeded,
        Message = message ?? "Refreshing the complete remaining exact-quality route.",
        UpdatedAtUtc = DateTimeOffset.UtcNow,
    });

    private void SetState(ExactAcquisitionRouteExecutionState state)
    {
        State = state;
        Persist();
    }
    ...
    private static void ValidateContractBinding(
        ...
        MarketAcquisitionClaimView claim)
    {
        if (contract.SchemaVersion != ExactAcquisitionExecutionContract.CurrentSchemaVersion ||
            contract.RecoveryPolicyId != ExactAcquisitionWorkbenchAuthority.CrossWorldExactQualityV1)
            throw new InvalidOperationException("Route cannot consume an unknown External plan contract or recovery policy version.");
        if (document.LocalRequestId != contract.WorkbenchDocumentId ||
            document.LocalRevision != contract.WorkbenchRevision ||
            ExactAcquisitionWorkbenchAuthorityService.ComputeCanonicalIntentHash(document) != contract.CanonicalIntentHash)
            throw new InvalidOperationException("The Workbench no longer matches the finalized External plan contract.");
        if (!string.Equals(claim.TargetCharacterName, contract.TargetCharacterName, StringComparison.Ordinal) ||
            ...
            throw new InvalidOperationException("The accepted work order targets a different character or world than the External plan contract.");
    }

    private static IReadOnlyList<ExactAcquisitionRouteLineProgress> BindLines(
        ExactAcquisitionExecutionContract contract,
        MarketAcquisitionClaimView claim) => contract.Lines.Select(envelope =>
    {
        ...
        return new ExactAcquisitionRouteLineProgress(
            line.LineId,
            envelope.ItemId,
            envelope.ItemName,
            envelope.Quality,
        ...
        bool remainingOnly = false)
    {
        if (plan.Status != "Ready")
            throw new InvalidOperationException("External plan preflight requires a ready route plan.");
        if (plan.WorldBatches.Count == 0)
            throw new InvalidOperationException("External plan preflight requires at least one executable world batch.");
        ...
        foreach (var line in lines)
        {
            ...
            plannedExternalGil = checked(plannedExternalGil + planned.PlannedGil);
        }
        if (plannedExternalGil > contract.PlanCapGil - lines.Aggregate(0ul, (sum, line) => checked(sum + line.SpentGil)))
            throw new InvalidOperationException("Prepared route exceeds the remaining confirmed external exact-acquisition plan ceiling.");
    }

    private static HashSet<string> ResolveAllowedWorlds(ExactAcquisitionExecutionContract contract)
    {
        if (contract.AuthorizedWorlds.Count == 0)
            throw new InvalidOperationException("The finalized External plan contract contains no authorized worlds.");
        ...
        if (contract.AuthorizedWorlds.Any(world => !regionalWorlds.Contains(world)))
            throw new InvalidOperationException("The finalized External plan contract contains a world outside its region.");
        return contract.AuthorizedWorlds.ToHashSet(StringComparer.OrdinalIgnoreCase);
    }

    internal static bool IsRestoredStateSane(
        ExactAcquisitionRouteExecutionState state,
        IReadOnlyList<ExactAcquisitionRouteLineProgress> bindings,
        ulong planCap)
    {
        if (state.SchemaVersion != ExactAcquisitionRouteExecutionState.CurrentSchemaVersion ||
    ...
    internal static bool IsRestoredStateSane(
        ...
        IsRestoredStateSane(state, BindLines(contract, claim), contract.PlanCapGil);
