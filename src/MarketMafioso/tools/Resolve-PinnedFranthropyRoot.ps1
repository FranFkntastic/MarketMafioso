function Resolve-PinnedFranthropyRoot {
    param(
        ...
        [string]$MarketMafiosoRepoRoot
    )

    $refPath = Join-Path -Path $MarketMafiosoRepoRoot -ChildPath "eng\franthropy.ref"
    ...
    $repository = [System.IO.Path]::GetFullPath((Join-Path -Path $MarketMafiosoRepoRoot -ChildPath "..\Franthropy"))
    if (-not (Test-Path -LiteralPath (Join-Path -Path $repository -ChildPath ".git"))) {
        throw "Franthropy was not found beside MarketMafioso at '$repository'."
    }
